...
async function hsRequest(path, method, accessToken, body) {
...
export async function captureContact(input, config) {
    ...
    const create = await hsRequest("/crm/v3/objects/contacts", "POST", config.accessToken, { properties });
    if (create.ok) {
    ...
    if (create.status === 409) {
        ...
        const update = await hsRequest(`/crm/v3/objects/contacts/${existingId}`, "PATCH", config.accessToken, { properties });
        if (update.ok) {
...
export function createClient(config) {
    return {
        capture: (input) => captureContact(input, config),
        isGenericEmail,
        extractDomain,
    };
}
